...
public interface PostRepository extends CrudRepository<PostEntity, Long> {
    ...
    @Query(value = "SELECT * FROM post WHERE u_id=:u_id" +
            " OR g_id IN (SELECT groups_id FROM users_groups WHERE members_id=:u_id)" +
            " OR page_id IN (SELECT pages_id FROM users_pages WHERE members_id=:u_id)" +
            " OR u_id IN (SELECT friends_id FROM users_friends WHERE user_entity_id=:u_id) " +
